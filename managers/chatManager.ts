import { Channel, Message, PendingMessageMeta, RealtimeMessageEvent, PresenceUser } from '@/.expo/types/chat';
...
interface MessagesState {
  ...
  typing: Record<string, { [userId: string]: { username?: string; lastTyping: string } }>; // channelId -> typing users
}
...
export class ChatManager {
  ...
  constructor(analytics: GameAnalytics) { this.analytics = analytics; }
  ...
  getMessages(channelId: string) { return this.state.byChannel[channelId] || []; }
  getUnread(channelId: string) { return this.state.unread[channelId] || 0; }
  getPresence(channelId: string) { return this.state.presence[channelId] || []; }
  getOnlineCount(channelId: string) { return this.getPresence(channelId).length; }
  ...
  setActiveChannel(channelId: string | null) {
    ...
    if (this.activeChannelId && this.activeChannelId !== channelId) {
      ...
      if (old) { old.unsubscribe(); this.subs.delete(this.activeChannelId); this.analytics.trackChatUnsubscribed(this.activeChannelId); }
      
      const oldPresence = this.presenceSubs.get(this.activeChannelId);
      if (oldPresence) { oldPresence.unsubscribe(); this.presenceSubs.delete(this.activeChannelId); }
      
      const oldTyping = this.typingSubs.get(this.activeChannelId);
      if (oldTyping) { oldTyping.unsubscribe(); this.typingSubs.delete(this.activeChannelId); }
    }
    ...
    if (channelId && !this.subs.has(channelId)) {
      ...
      this.analytics.trackChatSessionStart(channelId, { isDirect: !!this.channels.find(c => c.id === channelId)?.is_direct, memberCount: 0 });
    }
  }
  ...
  const prepend = messages.filter(m => !mergedIds.has(m.id)).map(m => ({ ...m, state: (m.deleted_at ? 'deleted' : 'sent') as Message['state'] }));
    this.state.byChannel[channelId] = [...prepend, ...existing];
    if (messages.length) this.state.paginationCursor[channelId] = messages[0].created_at; else this.state.paginationCursor[channelId] = null;
    return { reachedStart };
  }
  ...
  async sendMessage(channelId: string, body: string) {
    ...
    if (!this.state.byChannel[channelId]) this.state.byChannel[channelId] = [];
    this.state.byChannel[channelId].push(optimistic); this.sort(channelId);
  ...
  const msgs = this.getMessages(channelId).map(m => m.client_temp_id === tempId ? { ...(saved as Message), state: 'sent' as Message['state'] } : m);
      this.state.byChannel[channelId] = msgs; this.sort(channelId);
      return saved;
    } catch (e: any) {
  const msgs = this.getMessages(channelId).map(m => m.client_temp_id === tempId ? { ...m, state: 'failed' as Message['state'] } : m);
      this.state.byChannel[channelId] = msgs;
      throw e;
    }
  }
  ...
  retryFailed(channelId: string, tempId: string) {
    const msg = this.getMessages(channelId).find(m => m.client_temp_id === tempId && m.state === 'failed');
    if (msg) {
      return this.sendMessage(channelId, msg.body);
    }
  }
  ...
  getDebugInfo() {
    const totalMessages = Object.values(this.state.byChannel).reduce((sum, messages) => sum + messages.length, 0);
    
    return {
      activeChannelId: this.activeChannelId,
      totalChannels: this.channels.length,
      totalSubscriptions: this.subs.size,
      totalMessages,
      presenceChannels: this.presenceSubs.size,
      typingChannels: this.typingSubs.size,
