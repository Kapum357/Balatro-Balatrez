            ...
            {playerHand.length === 0 ? (
                ...
                <>
                    <View style={{marginBottom: 12}}>
                        <Text style={{fontWeight: 'bold'}}>Ante: {ante} | Blind: {blind.toUpperCase()} |
                            Objetivo: {target}</Text>
                        <Text>Manos restantes: {handsRemaining} | Cash: {cash}</Text>
                        {isWin &&
                            <Text style={{color: 'green', fontWeight: 'bold'}}>¡Has completado el Ante {ante - 1}! (Modo
                                infinito activo)</Text>}
                        {isOverflow &&
                            <Text style={{color: 'orange', fontWeight: 'bold'}}>Objetivo fuera de rango: progresión
                                detenida</Text>}
                    ...
                    {inShop ? (
                        ...
                        <>
                            ...
                            <Button title="Descartar Cartas Seleccionadas" onPress={handleDiscard}
                                    disabled={isGameOver || isOverflow || inShop}/>
