...
(
  ...
  ON DELETE CASCADE,
    last_typing TIMESTAMPTZ DEFAULT NOW
(
) NOT NULL,
...
CREATE
POLICY "Users can insert own typing state" ON public.typing
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE
POLICY "Users can update own typing state" ON public.typing
    FOR
UPDATE USING (auth.uid() = user_id);

CREATE
POLICY "Users can delete own typing state" ON public.typing
    FOR DELETE
USING (auth.uid() = user_id);

CREATE
POLICY "Users can view typing in accessible channels" ON public.typing
    FOR
SELECT USING (
    EXISTS (
...
CREATE
OR REPLACE FUNCTION cleanup_old_typing()
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
...
GRANT ALL
ON public.typing TO authenticated;
GRANT ALL
ON public.typing TO service_role;